/**
 * ============================================
 * AIR RECORD APP - Print Compressed Air
 * ============================================
 * Version: 1.0.0
 ...
 *   - 10 samples per page
 *   - Max 50 samples = up to 5 pages
 *   - All pages share same docNo
 * Tags: docNo, building, samplingDate, performedDate, incNo,
 *        lotTSA, lotOther, mfgMedia, expMedia,
 *        determinedDate, concludedDate, approvedDate, tempRoom01
 *        Per sample (x10): roomNo01-10, grade01-10, temp01-10, rh01-10, occResult01-10
 * ============================================
 */
...
// CONFIGURATION
// ============================================
const FORM_TYPE = 'COMPRESSED-AIR';
const TEMPLATE_NAME = 'ca-template.docx';
const FOLDER_NAME = 'compressed-air';
...
async function loadWorksheets() {
  ...
  try {
    ...
    const allData = await fetchAllAirWorksheets(scriptUrl, 'compressed-air');
    if (allData.length) {
...
async function selectWorksheet(index) {
  ...
  try {
    const response = await fetch(`${AIR_PRINT_CONFIG.SCRIPT_URL}?action=get&workflow=compressed-air&recordKey=${encodeURIComponent(summary.recordKey || summary.worksheetNo || summary.docNo)}`);
...
function previewData() {
  ...
  const html = `
    <div style="width: 100%; max-width: 900px;">
      ${generateCADocInfoHTML(selectedData)}
      ${generateCAMediaHTML(selectedData)}
      ${generateCASamplesTableHTML(samples)}
    </div>
  `;
...
function mapDataToTags(data, pageSamples) {
  ...
  json.lotOther = stripApostrophe(data.lotOther) || '';
  ...
  json.tempRoom01 = data.temp || '';

  // Samples (max 10 per page)
...
// ============================================
function generateCADocInfoHTML(data) {
  return `
...
function generateCAMediaHTML(data) {
  return `
    <section class="preview-section">
      <div class="preview-section__header">Media Information</div>
      <div class="preview-grid preview-grid--4">
        <div class="preview-field">
          <div class="preview-field__label">Lot TSA</div>
          <div class="preview-field__value">${data.lotTSA || '-'}</div>
        </div>
        <div class="preview-field">
        ...
        <div class="preview-field">
          <div class="preview-field__label">Lot Other</div>
          <div class="preview-field__value">${data.lotOther || '-'}</div>
        </div>
        <div class="preview-field">
          <div class="preview-field__label">Mfg / Exp</div>
          <div class="preview-field__value">${ensureDateFieldString(data.mfgMedia) || '-'} / ${ensureDateFieldString(data.expMedia) || '-'}</div>
        </div>
      </div>
    </section>
  `;
...
function generateCASamplesTableHTML(samples) {
