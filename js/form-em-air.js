/**
 * ============================================
 * AIR RECORD APP - EM Air Form Logic
 * ============================================
 * Version: 1.0.0
 * Dependencies: utils.js, db.js, sync.js, form-utils.js
 * 
 * Sheet: records_em (18 cols)
 * Database: database_em (6 cols)
 * Template: em-template.docx (50 samples/page, max 150)
 * DocNo: AT-YY-XXXX
 * ============================================
 */
...
document.addEventListener('DOMContentLoaded', async () => {
  console.log('Initializing EM Air Form...');
...
async function loadMasterDataFromSource(building) {
  console.log('🔄 Loading EM master data for:', building);

  try {
    if (AIR_SYNC_CONFIG.SCRIPT_URL) {
      const response = await fetch(`${AIR_SYNC_CONFIG.SCRIPT_URL}?action=getMasterData&sheetName=database_em`);
      ...
      if (result.success && result.data && result.data.length > 0) {
        ...
        window.samplingPoints = samplingPoints;
        console.log(`✅ Loaded ${filtered.length} EM sampling points from API`);
        return;
      }
    }
  } catch (error) {
  ...
  try {
    ...
    window.samplingPoints = samplingPoints;
    console.log(`✅ Loaded ${points.length} EM sampling points from local DB`);
  } catch (error) {
...
function updateSamplingPointDropdowns() {
  ...
  selects.forEach(select => {
    ...
    samplingPoints.forEach(point => {
      const option = document.createElement('option');
      option.value = point.noLocation || point.samplingPoint;
      option.textContent = point.noLocation || point.samplingPoint;
      select.appendChild(option);
    });

    if (currentValue && samplingPoints.some(p => (p.noLocation || p.samplingPoint) === currentValue)) {
...
// ============================================
// Fetch Next DocNo from Sheet (AT-YY-XXXX)
// ============================================
async function fetchNextDocNoFromSheet() {
  ...
  try {
    if (AIR_SYNC_CONFIG.SCRIPT_URL) {
      const url = `${AIR_SYNC_CONFIG.SCRIPT_URL}?action=getNextDocNo&status=${encodeURIComponent(currentStatus)}&formType=EM`;
...
async function generateLocalDocNo() {
  ...
  const storeKey = `lastSeq_AT_${year}`;
  const storedSeq = Storage.get(storeKey) || 0;
  const nextSeq = storedSeq + 1;

  const docNo = `AT-${year}-${String(nextSeq).padStart(4, '0')}`;
...
// Paste Handler
// ============================================
const EM_COLUMN_PREFIXES = ['tempRoom-', 'rhRoom-', 'timeIn-', 'timeOut-', 'result-', 'remark-'];
...
function handleColumnPaste(e) {
  ...
  const id = target.id || '';
  const prefix = EM_COLUMN_PREFIXES.find(p => id.startsWith(p));
...
async function handlePasteFromExcel(e) {
  ...
  // EM paste columns: location, tempRoom, rhRoom, timeIn, timeOut, result
...
async function processPastedData(pastedRows) {
  ...
  for (let i = 0; i < pastedRows.length; i++) {
    ...
    // Set EM-specific fields
    if (pastedRow.tempRoom) {
      const el = document.getElementById(`tempRoom-${rowId}`);
      if (el) el.value = pastedRow.tempRoom;
    }
    if (pastedRow.rhRoom) {
      const el = document.getElementById(`rhRoom-${rowId}`);
      if (el) el.value = pastedRow.rhRoom;
    }
    if (pastedRow.timeIn) {
      const el = document.getElementById(`timeIn-${rowId}`);
      if (el) el.value = pastedRow.timeIn;
    }
    if (pastedRow.timeOut) {
      const el = document.getElementById(`timeOut-${rowId}`);
      if (el) el.value = pastedRow.timeOut;
    }
    if (pastedRow.result) {
...
function addSampleRow() {
  ...
  if (isPasteMode) {
    row.innerHTML = `
      ...
      <td><input type="number" step="0.1" id="tempRoom-${rowId}" value="" style="width: 55px;"></td>
      <td><input type="number" step="0.1" id="rhRoom-${rowId}" value="" style="width: 55px;"></td>
      <td><input type="time" id="timeIn-${rowId}" style="width: 80px;"></td>
      <td><input type="time" id="timeOut-${rowId}" style="width: 80px;"></td>
      <td><input type="text" id="result-${rowId}" value="" placeholder="CFU" class="result-input" style="width: 55px;"></td>
    ...
    row.innerHTML = `
      ...
      <td class="col-point">
        <select class="sampling-point-select" onchange="onSamplingPointChange(this, '${rowId}')">
          ...
          ${samplingPoints.map(p => `<option value="${p.noLocation || p.samplingPoint}">${p.noLocation || p.samplingPoint}</option>`).join('')}
        </select>
      </td>
      <td class="col-location"><div class="info-display" id="location-${rowId}">-</div></td>
      <td><span class="info-display" id="grade-${rowId}">-</span></td>
...
// ============================================
// Sampling Point Change (auto-fill grade & floor)
// ============================================
...
function onSamplingPointChange(selectEl, rowId) {
  ...
  if (checkDuplicateRoomNo(value, rowId)) {
    UI.showToast(`⚠️ Room No. "${value}" ซ้ำกับแถวอื่นในตาราง`, 'warning');
  }

  const pointData = samplingPoints.find(p => (p.noLocation || p.samplingPoint) === value);
  if (pointData) {
    if (gradeEl) gradeEl.textContent = pointData.grade || '-';
    if (locationEl) locationEl.textContent = pointData.location || pointData.samplingPoint || 'ไม่พบ Location ที่ตรงกับ Room No.';
  } else {
    if (gradeEl) gradeEl.textContent = '-';
  ...
  rows.forEach((row, index) => {
    ...
    if (samplingPoint) {
      const pointData = samplingPoints.find(p =>
        (p.noLocation || p.samplingPoint) === samplingPoint ||
        (p.noLocation || p.samplingPoint).toLowerCase() === samplingPoint.toLowerCase()
      );
