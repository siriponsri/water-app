 ...
 */
async function generatePdfFilesLegacy(baseInfo, samples, mapFn, perPage) {
 ...
 */
// Canonical document pipeline override. Kept adjacent to the legacy helper so
// existing callers retain their interface while using /api/pdfs.
async function generatePdfFiles(baseInfo, samples, mapFn, perPage) {
  ...
  for (let p = 0; p < totalFiles; p++) {
    const pageSamples = samples.slice(p * perPage, (p + 1) * perPage);
    const mapped = mapFn(pageSamples);
    const tags = mapped && typeof mapped === 'object' ? mapped : {};
    pages.push(Object.assign({}, tags, { worksheetNo }));
    showProgress(p + 1, totalFiles, `Preparing PDF page ${p + 1} / ${totalFiles}...`);
  }
  const response = await fetch(`${PRINT_CONFIG.SERVER_URL}/api/pdfs`, {
    method: 'POST', headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ workflow: formType, worksheetNo, templateName, data: pages[0], pages })
  });
  if (!response.ok) {
  ...
  if (!result.pdfId) throw new Error('PDF generation returned no pdfId');
  showProgress(totalFiles, totalFiles, 'Complete!');
  await new Promise(r => setTimeout(r, 300));
  hideProgress();
  return [{ key: worksheetNo, pdfId: result.pdfId, url: `${PRINT_CONFIG.SERVER_URL}/api/pdfs/${result.pdfId}/download?inline=1` }];
}
...
// Canonical desktop save helper. Callers pass the pdfId returned by
// generatePdfFiles; the server owns filename identity and overwrite policy.
async function savePdfToDesktop(pdfId, formType) {
  const response = await fetch(`${PRINT_CONFIG.SERVER_URL}/api/pdfs/${encodeURIComponent(pdfId)}/save-desktop`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
