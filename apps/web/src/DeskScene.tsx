...
import { onQualityChange, readQuality, type QualityChoice } from './settings';
...
function Binder({ binder, x, lean, neighbourShift, paint, room, night, form, hovered, chosen, returning, arrivalKey, arrivalIndex, onHover, onOpen }: {
  ...
  /* Restarted when the building changes, so the whole row arrives together. */
  const arrival = useRef(0);
  useEffect(() => { arrival.current = 0; }, [arrivalKey]);
  ...
  useFrame((_, delta) => {
    ...
    /* Arrival. Changing building used to swap the binders in the same frame,
       which read as a glitch rather than as a shelf. Now each one is lifted
       in and set down, a beat after the one to its left — the eye follows the
       row instead of being asked to re-read a whole new set at once.

       The delay is short and the whole run is capped: past about half a
       second of staggering it stops reading as one movement and starts
       feeling like a queue the reader is waiting on. */
    /* arrivalIndex -1 means the reader asked for calm: the binders are simply
       on the shelf, with no arrival to watch. */
    const delay = arrivalIndex < 0 ? 0 : Math.min(arrivalIndex * 0.055, 0.34);
    arrival.current = arrivalIndex < 0 ? 1 : Math.min(1, arrival.current + delta / 0.42);
    const landed = arrivalIndex < 0 ? 1 : smooth(Math.max(0, Math.min(1, (arrival.current - delay) / 0.5)));

    const pivot = hovered ? 0.62 : 0;
    const rise = (1 - landed) * 0.55;
    node.position.x = mix(node.position.x, x + neighbourShift * 0.09, 0.2);
    node.position.y = mix(node.position.y, SHELF_Y + BINDER_H / 2 + (hovered ? 0.05 : 0) + rise, 0.2);
    ...
    /* It settles onto the shelf leaning slightly, then straightens — a binder
       put down by a hand, not a box snapped to a grid. */
    node.rotation.z = mix(node.rotation.z, lean + neighbourShift * 0.035 + (1 - landed) * 0.06, 0.2);
    if (landed < 1) node.scale.setScalar(mix(node.scale.x, 1, 0.2));
  });
...
/* =========================================================================
   Quality governor
   -------------------------------------------------------------------------
   The target machines are office PCs on Intel onboard graphics, and there is
   no way to know from a development machine how one of them will cope. So
   the scene measures itself on the machine it is actually running on, and
   steps down once if it is not keeping up.

   The 2048px shadow map is the expensive part by a wide margin — it is a
   second render of every caster, every frame. Halving it and then dropping
   it entirely is the cheapest large saving available, and a shelf without
   contact shadows still reads as a shelf. Resolution comes down with it.

   It only ever steps DOWN, and never within the first second: a first frame
   is always slow (shader compilation, texture upload) and reacting to it
   would demote every machine, including fast ones.
   ========================================================================= */
/** Keeps the renderer's shadow flag in step with the current quality. */
function ShadowSwitch({ on }: { on: boolean }) {
  const { gl } = useThree();
  useEffect(() => {
...
function QualityGovernor({ choice, onDemote }: { choice: QualityChoice; onDemote: (level: number) => void }) {
  const { gl } = useThree();
  ...
  useFrame((state, delta) => {
    /* "full" means the operator has told us this machine is fast enough and
       would rather keep the picture than have it quietly taken away. Honour
       that: measuring and then ignoring the result would only burn frames. */
    if (choice !== 'auto') return;
    const now = state.clock.elapsedTime;
    if (!startedAt.current) { startedAt.current = now; windowStart.current = now; }
    /* Never react to the first second: shader compilation and texture upload
       make it slow on every machine, fast ones included. */
    if (now - startedAt.current < 1) { windowStart.current = now; return; }
    if (level.current >= 2) return;

    samples.current.push(delta);

    /* Closed on frames OR on elapsed time, whichever comes first. Frames
       alone was backwards: at 1fps, waiting for 45 of them means waiting the
       better part of a minute before helping the machine that needs it most. */
    const enoughFrames = samples.current.length >= 45;
    const enoughTime = now - windowStart.current >= 1.5 && samples.current.length >= 5;
    ...
    /* Below ~30fps sustained, the shelf costs more than it gives. */
    if (median > 0.033) {
      level.current += 1;
      /* The shadow map is the expensive pass; step it down, then off. */
      gl.shadowMap.needsUpdate = true;
      onDemote(level.current);
      /* Observable from outside: a support call can ask what this says
         instead of guessing how the shelf is behaving on that PC. */
      try { document.documentElement.dataset.shelfQuality = String(level.current); } catch { /* ignore */ }
    }
  });
  return null;
}

function Room({ groups, activeGroup, hoveredBinder, chosen, returningBinder, night, calm = false, paletteRevision = 0, shape = 'lever-arch', onPickGroup, onHoverBinder, onOpenBinder }: {
  groups: DeskGroupView[];
  activeGroup: string;
  hoveredBinder: string | null;
  chosen: string | null;
  ...
  /* Two degrees of parallax. Enough to feel like a room you are standing in,
     not enough to become an effect — and the only motion here that runs
     without the reader doing anything, which is exactly why "fast" has to
     switch it off. Somebody who chose that setting because movement on the
     edge of their vision bothers them was previously given the same
     movement at a lower resolution. */
  useFrame(() => {
    ...
    <group ref={rig}>
      ...
      {binders.map((binder, index) => {
        ...
        return <Binder
          ...
          arrivalKey={activeGroup}
          arrivalIndex={calm ? -1 : index}
...
export default function DeskScene({
  ...
  /* 0 full, 1 reduced shadow map, 2 no shadows. In "auto" this is set by
     QualityGovernor from frame times measured on this machine; the other two
     choices pin it, which is the whole point of offering them. */
  const [choice, setChoice] = useState<QualityChoice>(readQuality);
  useEffect(() => onQualityChange(() => setChoice(readQuality())), []);
  const [measured, setMeasured] = useState(0);
  /* Cleared on mount: it records what THIS run of the shelf had to do, and a
     stale value from an earlier, slower session would have the settings page
     reporting a demotion that is not happening. */
  useEffect(() => {
    try { delete document.documentElement.dataset.shelfQuality; } catch { /* ignore */ }
  }, []);
  const quality = choice === 'fast' ? 2 : choice === 'full' ? 0 : measured;
  ...
  return (
    <Canvas
      shadows
      /* Tuned to the machine the laboratory actually bought: a Core Ultra 5
         235U, whose integrated part is Intel Graphics with 4 Xe-cores at up
         to 2.05 GHz — a different class from the old UHD parts this was
         first hedged against, and comfortably able to shade this scene at
         full device resolution. So the cap is the panel's own pixel ratio.

         Measured on the fleet: hardware acceleration is on, WebGL2 is
         available, and the scene's own probe pinned the vsync ceiling at
         both light and heavy load — so 60 fps is a floor here, not a limit.

         The floor of 1.5 is the point. The panels are 1920x1200 at a device
         pixel ratio of 1.25, and a cap alone would have rendered at 1.25 —
         native, with no supersampling, which is *softer* than the 1.5 this
         used to be hardcoded to. Raising a ceiling lowered the picture on
         every machine in the building. The floor buys back the antialiasing
         on a 1.25 panel and the cap keeps a retina display honest.

         Sustained clocks on a 15 W part are still not a given, so
         QualityGovernor stays: it is what makes this safe rather than a
         gamble. */
      /* A SCALAR, not a range. Given `dpr={[min, max]}` r3f clamps the
         device's own pixel ratio into that window, so a range can never
         render above native — the 1.5 asked for on a 1.25 panel came out as
         1.25, and the supersampling this exists for silently did not happen.
         Verified by reading canvas.width against clientWidth at the fleet's
         real 1920x1200 @1.25. */
      dpr={quality >= 2 ? 1 : quality === 1 ? 1.25 : Math.max(1.5, Math.min(2, typeof window === 'undefined' ? 1 : window.devicePixelRatio || 1))}
    ...
    >
      ...
      <FitCamera />
      <QualityGovernor choice={choice} onDemote={setMeasured} />
      {/* Shadows follow `quality` reactively rather than being switched off
          once and left off. The governor used to set gl.shadowMap.enabled
          directly, which nothing ever set back — so a later choice of "full"
          restored the resolution and the shadow map size but never the
          shadows themselves. */}
      <ShadowSwitch on={quality < 2} />
      ...
      <directionalLight
        ...
        castShadow
        shadow-mapSize={quality === 0 ? [2048, 2048] : [1024, 1024]}
